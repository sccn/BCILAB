...
opts=arg_define([0 3],varargin, ...
    arg_norep('trials'), ...
    arg_norep('targets'), ...
    arg({'lambdas','Lambdas'}, 10.^(1:-.5:-8), [], 'Vector of regularization parameters to consider. If this is specified as a vector instead of a search() expression, HKL will internally optimize the parameter using a more efficient procedure than the framework''s search() facility.','cat','Core Parameters'), ...
    ...
    ...
    arg({'maxactive','MaxKernelsActive'},400,[],'Maximum active kernels..','cat','Efficiency'), ...
    arg({'memory_cache','MemoryCache'},0.25,[],'Size of the memory cache. If < 1, assumed to be a fraction of the current free memory, otherwise in bytes. Should be large enough to fill most of the free RAM for efficiency.','cat','Efficiency'), ...
    arg({'k','Foldness'},5,[],'Cross-validation folds for parameter search.','cat','Efficiency'), ...
    ...
