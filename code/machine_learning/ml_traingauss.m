...
arg_define([0 2],varargin, ...
    arg_norep('trials'), ...
    arg_norep('targets'), ...
    arg({'dimensions','Dimensions'}, [], [0 Inf], 'Reduce data dimentions to n. If given, reduce data dimensionality to this number of dimensions using a strong feature selector.','shape','scalar'), ...
    arg({'blend','CovBlending'}, 1, [0 1], 'Degree of blending between per-class covariances. Blends covariance matrices (0=all covs independent, 1=all covs identical) and can be selected by a parameter search, e.g., by specifying search(0:0.1:1)'), ...
    ...
    arg({'startcost','InitialCost'},2,[0 Inf],'Initial cost for dimensionality reduction. If dimensionality reduction is requested, this is the initial cost which will be lowered until the desired number of dimensions is reached.'));
