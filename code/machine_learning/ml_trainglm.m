...
args = arg_define([0 2],varargin, ...
    ...
    arg({'lambdas','Lambdas'}, 1, [], 'Noise variance parameter. This can be used as a regularization parameter.'), ...
    ...
    arg({'outerNiter','OuterIterations'},10,[],'Outer loop iterations.'),...
    arg({'outerMVM','OuterLanczosVectors'},50,[],'Outer Lanczos vectors. Number of Lanczos vectors to compute; more may yields a more complete posterior estimation.'),...
    arg({'innerMVM','InnerMVMSteps'},50,[],'Inner-loop MVM steps. Number of matrix-vector multiplications and/or conjugate gradient steps to perform in inner loop.'),...
    arg({'innerIt','InnerNewtonSteps'},15,[],'Inner-loop MVM steps. Number of Newton steps to perform in inner loop (if Truncated-Newton is used as the solver).'),...
