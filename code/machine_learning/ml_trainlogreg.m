...
arg_define([0 3],varargin, ...
    ...
    arg_subswitch({'variant','Variant'},'vb', ...
        {'vb',{},'vb-iter',{},'vb-ard',{},'l1',{},'l2',{}, ...
         'lars',{arg({'nfolds','NumFolds'},5,[],'Cross-validation folds. The cross-validation is used to determine the best regularization parameter. If this is smaller than 1, it is taken as a fraction of the number of trials.'),...
                  ...
                  arg({'nlambda','NumLambdas'},100,[],'Number of lambdas. Number of lambda (regularization) parameter values to consider.','guru',true), ...
                  arg({'penalty_factor','PenaltyFactor'},[],[],'Penalty per feature. Allows for selective shrinkage and the like.'), ...
                  arg({'lambda_min','MinLambda'},0,[],'Minimum lambda. Smallest value for lambda, as a fraction of lambda_max, the (data derived) entry value (i.e., the smallest value for which all coefficients are zero). By default 0.0001, or 0.05 if #observations < #trials.','guru',true), ...
                  arg({'thresh','ConvergenceThreshold'},[],[],'Convergence threshold. Each inner coordinate-descent loop continues until the relative change in any coefficient is less than this (if specified).','guru',true), ...
                  arg({'dfmax','MaxVariables'},0,[],'Max #of variables. If specified, this constraints the maximum number of variables in the model.','guru',true), ...
                  arg({'pmax','MaxNonzeroes'},0,[],'Max #of non-zeroes. If specified, this constraints the maximum number of non-zero variables in the model.','guru',true), ...
