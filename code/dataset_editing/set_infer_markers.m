...
arg_define(varargin, ...
    ...
    arg({'max_events','MaxEvents'},30000,[], 'Number of events allowed. If a channel would produce more than this many events, it is not considered an event channel.'), ...
    arg({'max_event_fraction','MaxEventFraction'},0.3,[], 'Maximum fraction of events. If a channel would produce more events than this fraction of its total number of samples, the channel is not considered an event channel.'), ...
    arg({'max_types','MaxTypes'},300,[], 'Number of eventtypes allowed. If a channel would produce more than this many event types, it is not considered an event channel.'), ...
    ...
    arg({'omit_baseline_thresh','OmitBaselineThreshold'},0.8,[],'Baseline omission threshold. If the baseline event takes up more than this fraction of the data, it will not be encoded as events.'), ...
