...
arg_define(varargin, ...
    ...
    arg({'stddev_cutoff','Cutoff','StandardDevCutoff','cutoff'}, 5, [], 'StdDev cutoff for rejection. Data segments whose variance is beyond this cutoff from the distribution of variance across the recording are considered missing data.'), ...
    ...
    arg({'processing_delay','ProcessingDelay'}, 0.125, [], 'Signal delay, in seconds. This can be anywhere between 0 and WindowLength/2; if this is too low some sharp-onset artifacts will leave brief "knacks" in the data.'), ...
    arg({'max_dimensions','MaxDimensions'}, 0.66, [], 'Maximum dimensionality to reconstruct. Up to this many dimensions (or up to this fraction of dimensions) can be reconstructed for a given data segment. This is useful because the lower eigenvalues are usually not estimated very well.','guru',true), ...
