...
arg_define(varargin, ...
    ...
    arg({'min_corr','MinimumCorrelation'}, 0.5, [0 0.3 0.95 1], 'Minimum correlation between channels. If the measure falls below this threshold in some time window, the window is considered abnormal.'), ...
    arg({'ignored_quantile','IgnoredQuantile'}, 0.1, [0 0.05 0.15 1], 'Quantile of highest correlations ignored. Upper quantile of the correlation values that may be arbitrarily high without affecting the outcome - avoids problems with shorted channels.'), ...
    arg({'window_len','WindowLength'}, 2, [0 0.25 5 Inf], 'Window length to compute correlations. Length of the windows (in seconds) for which correlation is computed; ideally short enough to reasonably capture periods of global artifacts (which are ignored), but not shorter (for statistica reasons).'), ...
