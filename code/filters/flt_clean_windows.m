...
arg_define(varargin, ...
    ...
    arg({'window_len','WindowLength'}, 0.5, [], 'Window length. Window length that is used to check the data for artifact content. This is ideally as long as the expected time scale of the artifacts but short enough to allow for several 1000 windows to compute statistics over.'), ...
    arg({'window_overlap','WindowOverlap'}, 0.66, [], 'Window overlap fraction. The fraction of two successive windows that overlaps. Higher overlap ensures that fewer artifact portions are going to be missed (but is slower).'), ...
    arg_sub({'fit_params','ParameterFitting','parameter_fitting'}, {}, { ...
        ...
        arg({'num_bins','NumBins'}, 50, [], 'Kullback-Leibler binning. Number of bins for Kullback-Leibler divergence calculation; for less than 3000 data windows, it may help to reduce the number of bins.','guru',true) ...    
...
for c = C:-1:1
    ...
    if isempty(flag_quantile)
        [mu,sig] = hlp_diskcache('statistics',@fit_eeg_distribution,X, ...
            fit_params.min_clean_fraction, fit_params.max_dropout_fraction, ...
            fit_params.fit_quantiles, fit_params.step_sizes, fit_params.num_bins);
    else
        [mu,sig] = deal(0,1);
    end
