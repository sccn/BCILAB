...
arg_define(varargin,...
    ...
    arg({'windowlength','WindowLength'},0.5,[],'Moving window length. Length of the moving window within which the spectrum shall be calculated (in seconds).'), ...
    arg({'windowstep','WindowStep'},0.1,[],'Moving window stepping. Step size by which the moving window shall be moved (in seconds).'), ...
    arg({'freqrange','FrequencyRange'},[2 48],[],'Frequency range. The frequency range for which the ERSP shall be calculated.'), ...
    ...
    arg({'numtapers','Tapers','tapers'},[],[],'Number of tapers. Should be an integer smaller than 2*TimeBandwith; default 2*TimeBandwidth-1','guru',true), ...
