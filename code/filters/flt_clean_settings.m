...
make_clean_params = @(flatlines,drifts,regression,windows,channels,dropouts,peaks,spikes,shaping) { ...
    ...
    arg_subtoggle({'channels','BadChannelRemoval','ChannelCleaning'},channels,@flt_clean_channels,'Rejection of channels with uncorrelated signals.'), ...
    ...
    arg_subtoggle({'pcasubspace','BadSubspaceRemoval','BurstRepair','bursts'},peaks,@flt_repair_bursts,'Rejection of high-power subspaces per window.'), ...
    arg_subtoggle({'pcaspikes','SpikeSubspaceRemoval','spikes'},spikes,@flt_repair_bursts,'Rejection of spike subspaces. Do not use for now (we need a slightly different method for this).','experimental',true), ...
    arg_subtoggle({'windows','BadWindowRemoval','WindowCleaning'},windows,@flt_clean_windows,'Rejection of time windows with excessive signal power.'), ...
    arg_subtoggle({'shaping','SpectrumShaping'},shaping,@flt_fir,'Reshaping of the signal spectrum. Done using a FIR filter.','experimental',true)};

% define arguments
arg_define(varargin, ...
    arg_norep({'signal','Signal'}), ...
    arg_subswitch({'cleansetting','DataSetting'},'1.1-beta', ...
    ...
    arg_deprecated({'reference_len','LinearReferenceLength'},3,[],'Linear reference length. The length of the assumed temporal dependencies between artifact channel content and EEG channel contents, in samples. Can get slow if this is very long (e.g., when removing entire VEPs).'), ...
