...
% read options
o = arg_define(varargin, ...
    arg({'scan_interval','ScanInterval'},15,[0.1 15 3600 1000000],'Interval between scans for changes. In seconds.'), ...
    arg({'wait_period','WaitPeriod'},120,[0 60 300 3600],'Wait this long for further changes until rebuild.'), ...
    arg({'scan_dirs','ScanDirectories'},{'bcilab:/code','bcilab:/*.m'},[],'Directories/files to scan for changes.'), ...
    arg({'project_name','ProjectName'},'bcilab:/build',[],'Name/location of the build project.'), ...
    arg({'spin_interval','SpinInterval'},5,[1 5 5 60 3600],'Check for finished build every this many seconds.'), ...
    arg({'log','LogFile'},'home:/.bcilab/logs/buildslave.log',[],'Name/location of log file.'), ...
    arg({'issue_build','IssueBuild'},'@env_compile_bcilab',[],'Function called to issue new build.','type','expression'), ...
    arg({'post_build','PostBuild'},[],[],'Function called after successful build.','type','expression'), ...
    arg({'wait_precompile','WaitPreCompile'},30,[1 10 60 300],'Assume compilation ramp-up takes this long.'), ...
    arg({'wait_postcompile','WaitPostCompile'},30,[1 10 60 300],'Assume compilation finalization takes this long. After mcc instances have finished.'), ...
    arg({'wait_postconflict','WaitPostConflict'},120,[1 30 360 3600],'Time to wait after a failed build. For instance, due to concurrent editing.'));
