...
% sample Weight [1 x N]                 : per-sample weight vector
%
% EXAMPLE
%       [E, D] = pcamat(vectors);
...
summedWeight = sum(sampleWeight);
repSampleWeight = repmat(sampleWeight,1,vectorSize);
...
if approachMode == 1,
    ...
    %ooNumSamplesA1 = GPUdouble(a1/numSamples);
    %ooNumSamples = GPUdouble(1/numSamples);
    %a1Xprime = GPUsingle(a1*X');
    %X = GPUsingle(X);
    numSamples = GPUdouble(numSamples);
    if strcmp(class(X),'single')
        X = GPUsingle(X);
    else
        X = GPUdouble(X);
    end
    oneB1 = GPUdouble(ones(size(B,1),1));
    ...
    for round = 1:maxNumIterations + 1,
        ...
        switch usedNlinearity
            ...
            case 10
                ...
                %B = (X * (repSampleWeight.*(( X' * B) .^ 3))) / summedWeight - 3 * B;
            case 11
                % optimoitu - epsilonin kokoisia eroja
                % t�m� on optimoitu koodi, katso vanha koodi esim.
                % aikaisemmista versioista kuten 2.0 beta3
                if summedWeight ~= numSamples
                    error('weighting not supported for this nonlinearity'); end
                Y = X' * B;
                Gpow3 = Y .^ 3;
                Beta = sum(Y .* Gpow3);
            ...
            case 20
                hypTan = tanh(a1 * X' * B);
                B = X * hypTan / numSamples - oneB1 * sum(1 - hypTan .^ 2) .* B / numSamples * a1;
                %B = X * (repSampleWeight.*hypTan) / summedWeight - ones(size(B,1),1) * (sum(repSampleWeight.*(1 - hypTan .^ 2))) .* B / summedWeight * a1;
            case 21
                if summedWeight ~= numSamples
                    error('weighting not supported for this nonlinearity'); end
        ...
        while i <= maxNumIterations + gabba
            ...
            switch usedNlinearity
                ...
                case 10
                    ...
                    %w = (X * (sampleWeight.*((X' * w) .^ 3))) / summedWeight - 3 * w;
...
%                       'skew'             g(u)=u^2
                ...
                case 11
                    ...
                    %EXGpow3 = (X * (sampleWeight.*((X' * w) .^ 3))) / summedWeight;
                    Beta = w' * EXGpow3;
                ...
                case 20                    
                    ...
                    %w = (X * (sampleWeight.*hypTan) - a1 * sum(sampleWeight.*(1 - hypTan .^ 2))' * w) / summedWeight;
                case 21
                    if summedWeight ~= numSamples
                        error('weighting not supported for this nonlinearity'); end
                    hypTan = tanh(a1 * X' * w);
