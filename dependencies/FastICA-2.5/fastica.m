...
%
% 'sampleWeight'        [1 x N] vector of per-sample weights
%
%
%======================================================================
...
sampleWeight = ones(1,NumOfSampl);
usegpu = 0;
...
if only > 2
    ...
    t0 = tic;
    if usegpu
        [A, W] = fpica_gpu (whitesig,  whiteningMatrix, dewhiteningMatrix, approach, ...
            numOfIC, g, finetune, a1, a2, myy, stabilization, epsilon, ...
            maxNumIterations, maxFinetune, initState, guess, sampleSize, ...
            displayMode, displayInterval, verbose,sampleWeight);
    else
    ...
    disp(['Time spent in kernel: ' num2str(toc(t0))]);
