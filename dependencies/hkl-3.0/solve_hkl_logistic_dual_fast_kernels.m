function [alpha,b,eta,zeta,kappa] = solve_hkl_square_dual_fast_kernels(eta,data,Y,lambda,varargin);
...
% optimization parameter for dual logistic regression
optparam.tol = 1e-16;
optparam.kmax = 2;
...
while k<=kmax;
	...
	[alphaK,exit_parameters] = minimize_newton(alphaK,@logistic_cost_dual,optparam, K,Y,lambda);
	fx = -logistic_cost_dual(alphaK,K,Y,lambda);
	...
	while ialpha<kmax_ARMIJO
		...
		K = center_gram_matrix(double(devectorize_single( single(data.kernels * zeta ))));
		[alphaK0,exit_parameters] = minimize_newton(alphaK,@logistic_cost_dual,optparam, K,Y,lambda);
		fxalpha = -logistic_cost_dual(alphaK0,K,Y,lambda);
...
K = center_gram_matrix(double(devectorize_single( single(data.kernels * zeta ))));
[alpha,exit_parameters] = minimize_newton(alphaK,@logistic_cost_dual,optparam, K,Y,lambda);
