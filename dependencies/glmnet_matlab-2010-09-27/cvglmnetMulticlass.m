function CVerr = cvglmnetMulticlass(x,y,nfolds,cvmargin,foldid,type,family,options,verbose,loss)
...
% the specific alpha. 
% Call: CVerr = cvglmnet(x,y,nfolds,cvmargin,foldid,type,family,options,verbose)
% Example:
% x=randn(100,2000);
% y=randn(100,1);
% g2=randsample(2,100,true);
% CVerr=cvglmnet(x,y,100,0,[],'response','gaussian',glmnetSet,1);
% CVerr=cvglmnet(x,g2,100,0,[],'response','binomial',glmnetSet,1);
% x         : Covariates: N*D
...
% loss      : loss measure to use; if default, chosen according to the distribution type,
%             (mcr or mse) otherwise using ml_calcloss to evaluate the loss (default: 'default')
% 
...
glmnet_object = hlp_diskcache('predictivemodels',@glmnet,x,y,family,options);
...
if strcmp(loss,'default')
    ...
    % custom loss
    for k=1:size(yy,2)
        cvraw(k) = ml_calcloss(loss,yy(:,k),predmat(:,k)); end
    CVerr.cvm=cvraw;
    % if there are several minima, choose largest lambda of the smallest cvm
    CVerr.lambda_min=max(options.lambda(CVerr.cvm<=min(CVerr.cvm)));
end
