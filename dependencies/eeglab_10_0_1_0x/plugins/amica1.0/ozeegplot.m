% ozeegplot() - Scroll (horizontally and/or vertically) through multichannel data.
...
if ~isstr(data) % If NOT a 'noui' call or a callback from uicontrols
   ...
   %catch
   %		disp('ozeegplot() error: calling convention {''key'', value, ... } error'); return;
   %end;	
  ...
  % ---------------------------------
  defdowncom   = 'ozeegplot(''defdowncom'',   gcbf);'; % push button: create/remove window
  defmotioncom = 'ozeegplot(''defmotioncom'', gcbf);'; % motion button: move windows or display current position
  defupcom     = 'ozeegplot(''defupcom'',     gcbf);';
  defctrldowncom = 'ozeegplot(''topoplot'',   gcbf);'; % CTRL press and motion -> do nothing by default
  ...
  defctrlupcom = ''; % CTRL press and up -> do nothing by default
   ...
   try, g.title; 		    catch, g.title		= ['Scroll activity -- ozeegplot()']; 	end;
   ...
   try, g.modelactive; 			catch, g.modelactive  = []; 	end;%AMICA
   try, g.curpatch;         catch, g.curpatch = [];    end;%AMICA
   ...
   %-------------AMICA-----------------------
   if ~isempty(g.modelactive)
       if size(g.modelactive,3)>1
           y = zeros(1,size(g.modelactive,3)*size(g.modelactive,2));
           for i = 1:size(g.modelactive,3);
               y((i-1)*size(g.modelactive,2)+1:i*size(g.modelactive,2)) = g.modelactive(1,:,i);
           end
           g.modelactive = y;
           clear y;
       end
   ...
   for index=1:length(gfields)
      ...
      case {'spacing', 'srate' 'eloc_file' 'winlength' 'position' 'title' ...
               ...
               'ploteventdur' 'butlabel' 'scale' 'events' 'data2' 'plotdata2' 'mocap' 'selectcommand' 'ctrlselectcommand' 'modelactive' 'curpatch' 'datastd' 'normed' 'envelope'},;
      otherwise, error(['ozeegplot: unrecognized option: ''' gfields{index} '''' ]);
      end;
   end;
  ...
  if isstr(g.eloc_file) | isstruct(g.eloc_file)  % Read in electrode names
      ...
      ozeegplot('setelect', g.eloc_file, ax1);
  end;
...
% positions of buttons
  ...
  posbut(22,:) = [0.738    0.93    0.06      0.048];%combine channels(same offset)
  posbut(23,:) = [-0.18    0.9    0.03      0.05]; %model legend
...
% Five move buttons: << < text > >> 
  ...
  u(1) = uicontrol('Parent',figh, ...
	...
	'Tag','Pushbutton1',...
	'string','<<',...
	'Callback','ozeegplot(''drawp'',1)');
  u(2) = uicontrol('Parent',figh, ...
	'Units', 'normalized', ...
	'Position', posbut(2,:), ...
	'Tag','Pushbutton2',...
	'string','<',...
	'Callback','ozeegplot(''drawp'',2)');
  u(5) = uicontrol('Parent',figh, ...
	...
	'Callback', 'ozeegplot(''drawp'',0);' );
  u(3) = uicontrol('Parent',figh, ...
	'Units', 'normalized', ...
	'Position',posbut(3,:), ...
	'Tag','Pushbutton3',...
	'string','>',...
	'Callback','ozeegplot(''drawp'',3)');
  u(4) = uicontrol('Parent',figh, ...
	'Units', 'normalized', ...
	'Position',posbut(4,:), ...
	'Tag','Pushbutton4',...
	'string','>>',...
	'Callback','ozeegplot(''drawp'',4)');
...
% ESpacing buttons: + -
  ...
  u(8) = uicontrol('Parent',figh, ...
	...
	'string','-',...
	'FontSize',8,...
	'Callback','ozeegplot(''draws'',2)');

cb_normalize = ['g = get(gcbf,''userdata'');'...
    ...
    'if g.normed, disp(''Denormalizing...'');for i = 1:size(data,1), data(i,:,:) = data(i,:,:)*g.datastd(i);set(gcbo,''string'', ''Norm'');set(findobj(''tag'',''ESpacing'',''parent'',gcbf),''string'',num2str(g.oldspacing));end;' ...
    'else, disp(''Normalizing...'');for i = 1:size(data,1), data(i,:,:) = data(i,:,:)/g.datastd(i);end;set(gcbo,''string'', ''Denorm'');g.oldspacing = g.spacing;set(findobj(''tag'',''ESpacing'',''parent'',gcbf),''string'',''5'');end;' ...
    'g.normed = 1 - g.normed;' ...
    'ozeegplot(''draws'',0);'...
...
u(22) = uicontrol('Parent',figh, ...
  ...
  if ~isempty(g.events)
      u(17) = uicontrol('Parent',figh, ...
                        'Units', 'normalized', ...
                        'Position',posbut(17,:), ...
                        'string', 'Event types', 'callback', 'ozeegplot(''drawlegend'', gcbf)');
  end;
  
  %-------------AMICA----------------------
  if ~isempty(g.modelactive)
       allcolors = 'bgrymcw';
       maxi = max(g.modelactive);
       str = {};
       hps = [];
       for i = 1:maxi
           str = {str{:} ['Mod ' num2str(i)]};
           hp = patch([1 1 1 1],[1 1 1 1],allcolors(i),'FaceAlpha',0.2);
           hps = [hps hp];
       end
       hlegend = legend(hps,'String',str,'Position',posbut(23,:));
  ...
  uimenu('Parent',m(7),'Label','Edit figure', 'Callback', 'ozeegplot(''noui'');');
  ...
  % Scale Eye %%%%%%%%%
  timestring = ['[OBJ1,FIG1] = gcbo;',...
	        'ozeegplot(''scaleeye'',OBJ1,FIG1);',...
		'clear OBJ1 FIG1;'];
  m(7) = uimenu('Parent',m(1),'Label','Show scale','Callback',timestring);
  
  % Title %%%%%%%%%%%%
  uimenu('Parent',m(1),'Label','Title','Callback','ozeegplot(''title'')')
  ...
  uimenu('Parent',m(6),'Label','Load .loc(s) file',...
      'Callback','ozeegplot(''loadelect'');')
  
  % Zooms %%%%%%%%
  zm = uimenu('Parent',m(2),'Label','Zoom off/on');
  commandzoom = [ 'set(gcbf, ''windowbuttondownfcn'', [ ''zoom(gcbf,''''down''''); ozeegplot(''''zoom'''', gcbf, 1);'' ]);' ...
  ...
  uimenu('Parent',zm,'Label','Zoom off', 'separator', 'on', 'callback', ...
     ...
     'clear tmpg;' ]);
  uimenu('Parent',figh,'Label', 'Help', 'callback', 'pophelp(''ozeegplot'');');

  % Events %%%%%%%%
  zm = uimenu('Parent',m(2),'Label','Events');
  complotevent = [ 'tmpg = get(gcbf, ''userdata'');' ...
                  'tmpg.plotevent = ''on'';' ...                  
                  'set(gcbf, ''userdata'', tmpg); clear tmpg; ozeegplot(''drawp'', 0);'];
  comnoevent   = [ 'tmpg = get(gcbf, ''userdata'');' ...
                  'tmpg.plotevent = ''off'';' ...                  
                  'set(gcbf, ''userdata'', tmpg); clear tmpg; ozeegplot(''drawp'', 0);'];
  comeventleg  = [ 'ozeegplot(''drawlegend'', gcbf);'];
...
%  set(figh, 'busyaction', 'cancel');
  ...
  ozeegplot('scaleeye', [], gcf);
  if strcmp(lower(g.scale), 'off')
	  ozeegplot('scaleeye', 'off', gcf);
  end;
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  case 'drawp' % Redraw EEG and change position
    ...
    % this test help to couple ozeegplot windows
    if exist('p3')
    	figh = p3;
    	figure(p3);
    else	
    ...
    % channels in the ozeegplot figure window.
    for i = 1:g.chans
...
%        keyboard;  
    ...
    set(ax1, 'ylim',[g.elecoffset*g.spacing (g.elecoffset+g.dispchans+1)*g.spacing] );
	 ...
	 if g.children ~= 0
		...
		ozeegplot( 'drawp', p1, p2, g.children);
		figure(figh);
	 end;	  
...
%      allcolors = 'rgbymcw';
%      if ~isempty(g.modelactive)
%         n_lgnd = max(g.modelactive);
%         str = {};
%         for i = 1:n_lgnd
%             temp_str = ['Model ' num2str(i)];
%             str = {str{:} temp_str}; 
%         end
%         lgnd = legend(str,'Position',[0.742    0.7    0.05      0.05]);
%      end
  ...
  case 'drawb' % Draw background ******************************************************
    ...
    if ~isempty(g.modelactive)
        allcolors = 'bgrymcw';
        colors = allcolors(1:max(max(g.modelactive)));
        if isempty(g.curpatch)
            
            
            startpoint = lowlim;
            temphighlim = min(highlim,length(g.modelactive));
            for i = lowlim+1:temphighlim
                if g.modelactive(i)~=g.modelactive(i-1)
                    hp = patch([startpoint-lowlim i-1-lowlim i-1-lowlim startpoint-lowlim],[0 0 1 1],colors(g.modelactive(i-1)),'FaceAlpha',0.2);
                    set(hp, 'EdgeColor', get(hp, 'facecolor'))
                    g.curpatch = [g.curpatch startpoint-lowlim i-1-lowlim i-1-lowlim startpoint-lowlim];
                    startpoint = i;
                    
                end
            end
            hp = patch([startpoint-lowlim highlim-lowlim highlim-lowlim startpoint-lowlim],[0 0 1 1],colors(g.modelactive(i-1)),'FaceAlpha',0.2);
            set(hp, 'EdgeColor', get(hp, 'facecolor'))
            g.curpatch = [g.curpatch startpoint-lowlim i-1-lowlim i-1-lowlim startpoint-lowlim g.modelactive(i-1) ];
        else
            for i = 1:length(curpatch)/5
                hp = patch(g.curpatch((i-1)*5+1:5*i-1),[0 0 1 1],colors(g.curpatch(5*i)),'FaceAlpha',0.2);
                set(hp, 'EdgeColor', get(hp, 'facecolor'))
            end
        end
...
{ 'Number of channels to display:' } , 'Change number of channels to display', 1,  { num2str(g.dispchans) });
   ...
   set(gcf, 'UserData', g);
   ozeegplot('updateslider', fig);
   ozeegplot('drawp',0);	
   ozeegplot('scaleeye', [], fig);
   return;
  ...
  case 'loadelect' % load channels
	...
	ozeegplot('setelect',[ inputpath inputname ],AXH0);
	return;
  ...
  case 'title'
	...
	if ~isempty(h)
		...
		if ~isempty(result), h = textsc(result{1}, 'title'); set(h, 'tag', 'ozeegplottitle');end;
	end;
  ...
  case 'noui'
   ...
   case 'zoom' % if zoom
      ...
      % ------------------------------
      if exist('p2') == 1
          set(gcbf, 'windowbuttondownfcn', [ 'zoom(gcbf,''down''); ozeegplot(''zoom'', gcbf, 1);' ]);
  ...
  case 'defmotioncom'
    ...
    if g.incallback
        ...
        ozeegplotdata = get(ax1, 'userdata');
        set(hh, 'string', num2str(ozeegplotdata(g.chans+1-tmpelec, min(g.frames,max(1,double(round(tmppos(1)+lowlim)))))));  % put value in the box
    ...
    if strcmp(get(fig, 'SelectionType'),'normal');
        ...
        if g.incallback ~= 1 % interception of nestest calls
            ...
            if (tmppos(1) >= 0) & (tmppos(1) <= highlim),
                ...
                set(fig,'UserData', g);
                ozeegplot('drawp', 0);  % redraw background
            end;
        end;
    end;      
   otherwise
      error(['Error - invalid ozeegplot() parameter: ',data])
  end  
end
