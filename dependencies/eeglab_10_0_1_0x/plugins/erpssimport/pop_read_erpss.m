...
% also add remaining event fields, if any...
fnames = setdiff(fieldnames(events),{'event_code','sample_offset'});
try
    for fn = fnames'
        fname = fn{1};
        [EEG.event.(fname)] = events.(fname);
    end
catch
    disp('Error in pop_read_erpss: Could not read out misc event fields; skipping...');
end
