...
if nargin < 2
  ...
  if ~isempty(EEG.dipfit.mrifile) && exist('spm_smooth','file')
      mri = EEG.dipfit.mrifile;
      if ischar(EEG.dipfit.mrifile)
          try
            mri = load(mri,'mri');
            mri = mri.mri;
          catch
      ...
      EEGOUT = dipfit_gridsearch(EEG, 'component', select, 'mri',mri, options{:});      
  else
    disp('Note: Your dipfit solutions will not be constrained to gray-matter locations unless you install the SPM 2 or 8 toolbox.');
