function STUDY = std_apcluster(STUDY,ALLEEG, numberOfClusters, outlierSTD, measuresToUseInClustering, methodParameter, conditionsToInclude )
% std_mpcluster() - Clusters STUDY ICs using Affinity Product method. 
...
% 
% Usage:   
%           >> STUDY = std_apcluster(STUDY,ALLEEG, numberOfClusters, outlierSTD, .. 
...
% Example:
%     
%     >> STUDY = std_apcluster(STUDY,ALLEEG, 10, Inf, {'dipole' ,'erp' ,'ersp'}, 0.8);    
...
%              
%  See also  apclusterK(),  std_apreclust(), pop_apcluster(),  pop_apreclust()
...
% if Affinity Product matrices are not present, calculate them
...
fprintf('Clustering with %s Affinity Product method into %d clusters. Measures to be used:\n', clusteringMethod, numberOfClusters);
...
% remove all clusters, then use std_checkset to create a new parent cluster
STUDY.cluster = [];
[STUDY ALLEEG]= std_checkset(STUDY, ALLEEG);

% add clusters (after parent cluster = 1)
STUDY  = std_createclust(STUDY,ALLEEG, 'clusterind', IDX,'algorithm', measuresToUseInClustering);
