...
if ~isstruct(INEEG2) % if INEEG2 is a vector of ALLEEG indices
    ...
    if INEEG1.nbchan ~= INEEG2.nbchan
        error('The two datasets (%s and %s) must have the same number of channels: ',[INEEG1.filepath filesep INEEG1.filename],[INEEG1.filepath filesep INEEG1.filename]);
    end;
    if INEEG1.srate ~= INEEG2.srate
        error('The two datasets (%s and %s) must have the same sampling rate',[INEEG1.filepath filesep INEEG1.filename],[INEEG1.filepath filesep INEEG1.filename]);
    end;
    if INEEG1.trials > 1 | INEEG2.trials > 1
        if INEEG1.pnts ~= INEEG2.pnts
            error('The two epoched datasets (%s and %s) must have the same number of points',[INEEG1.filepath filesep INEEG1.filename],[INEEG1.filepath filesep INEEG1.filename]);
        end;
        ...
        if isfield(INEEG2, 'urevent')
            if ~isempty(INEEG2.urevent)
                ...
                try
                    ...
                    fprintf('Could not concatenate urevents properly...\n');
                end
            else
