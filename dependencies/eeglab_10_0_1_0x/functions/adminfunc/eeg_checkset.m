...
if isempty(defaults)
    ...
    defaults.fieldorder = { 'setname' 'filename' 'filepath' 'subject' 'group' 'condition' 'session' 'comments' 'nbchan' 'trials' 'pnts' 'srate' 'xmin' 'xmax' ...
        'times' 'data' 'icaact' 'icawinv' 'icasphere' 'icaweights' 'icachansind' 'chanlocs' 'urchanlocs' 'chaninfo' 'ref' 'event' 'urevent' ...
        'eventdescription' 'epoch' 'epochdescription' 'reject' 'stats' 'specdata' 'specicaact' 'splinefile' 'icasplinefile' 'dipfit' 'history' 'saved' 'etc'};
    % default .stats field
    defaults.stats = struct('jp',{[]},'jpE',{[]},'icajp',{[]},'icajpE',{[]},'kurt',{[]},'kurtE',{[]},'icakurt',{[]},'icakurtE',{[]},'compenta',{[]},'compentr',{[]},'compkurta',{[]},'compkurtr',{[]},'compkurtdist',{[]});
    defaults.statsfields = {'jp','jpE','icajp','icajpE','kurt','kurtE','icakurt','icakurtE','compenta','compentr','compkurta','compkurtr','compkurtdist'};
    % default .reject field
    defaults.reject = struct('rejmanual', {[]}, 'rejjpE', {[]}, 'rejjp', {[]}, 'rejkurtE', {[]}, 'rejkurt', {[]}, 'rejmanualE', {[]}, 'rejthreshE', {[]}, 'rejthresh', {[]}, 'rejconstE', {[]}, 'rejconst', {[]}, 'rejfreqE', {[]}, 'rejfreq', {[]}, 'icarejjpE', {[]}, 'icarejjp', {[]}, 'icarejkurtE', {[]}, 'icarejkurt', {[]}, 'icarejmanualE', {[]}, 'icarejmanual', {[]}, 'icarejthreshE', {[]}, 'icarejthresh', {[]}, 'icarejconstE', {[]}, 'icarejconst', {[]}, 'icarejfreqE', {[]}, 'icarejfreq', {[]}, 'rejglobal', {[]}, 'rejglobalE', {[]}, 'rejmanualcol', {[1 1 0.783]}, 'rejthreshcol', {[0.8487 1 0.5008]}, 'rejconstcol', {[0.694 1 0.7008]}, 'rejjpcol', {[1 0.6991 0.7537]}, 'rejkurtcol', {[0.688 0.7042 1]}, 'rejfreqcol', {[0.9596 0.7193 1]}, 'disprej', {{}}, 'threshold', {[0.8 0.8 0.8]}, 'threshentropy', {600}, 'threshkurtact', {600}, 'threshkurtdist', {600}, 'gcompreject', {[]});
    defaults.rejectfields = {'rejmanual', 'rejjpE', 'rejjp', 'rejkurtE', 'rejkurt', 'rejmanualE', 'rejthreshE', 'rejthresh', 'rejconstE', 'rejconst', 'rejfreqE', 'rejfreq', 'icarejjpE', 'icarejjp', 'icarejkurtE', 'icarejkurt', 'icarejmanualE', 'icarejmanual', 'icarejthreshE', 'icarejthresh', 'icarejconstE', 'icarejconst', 'icarejfreqE', 'icarejfreq', 'rejglobal', 'rejglobalE', 'rejmanualcol', 'rejthreshcol', 'rejconstcol', 'rejjpcol', 'rejkurtcol', 'rejfreqcol', 'disprej', 'threshold', 'threshentropy', 'threshkurtact', 'threshkurtdist', 'gcompreject'};
end
...
% 		        end;
    ...
    % -----------------------
    if ~isfield(EEG, 'stats') || isempty(EEG.stats) || isequal(EEG.stats,struct('jp',{[]}))
        EEG.stats = defaults.stats;
    elseif ~isequal(fieldnames(EEG.stats)',defaults.statsfields)
    ...
    % component rejection
    % -------------------    
    if ~isfield(EEG, 'reject') || isempty(EEG.reject) || isequal(EEG.reject,struct('rejmanual',{[]}))
        EEG.reject = defaults.reject;
    else
        if ~isequal(EEG.reject,defaults.reject)
...
% recorder fields, if necessary
% -----------------------------
try
    fnames = fieldnames(ALLEEGNEW)';
    if ~isequal(fnames(1:length(defaults.fieldorder)),defaults.fieldorder)    
        fieldorder = [defaults.fieldorder fast_setdiff(fnames,defaults.fieldorder)];    
...
% new object support
% ------------------
if option_eegobject && ~isa(EEG, 'eegobj')
    EEG = eegobj(EEG);
end;
