...
function [m,ga,b,z,nlZ,varargout] = dli(X,y,s2,B,pot,tau, opts,G)
...
if isfield(opts,'outerMVM')       % number of outer loop MVMs or Lanczos vectors
  noutMVM = opts.outerMVM;
else
  noutMVM = 50;
end
if isfield(opts,'outerExact')  % Do we try and compute exact marginal variances?
  outerExact = opts.outerExact;         % B has to be the identity and X numeric
else
  outerExact = 0;
end
...
q = numel(B*u);
z = z0*ones(q,1); ldA = 0; nlZ = Inf;
if isscalar(G)
    b = feval(pot,zeros(q,1)); b = b(:,4);
else
    b = feval(pot,G*zeros(q,1)); b = b(:,4);
end
varargout = cell(nargout-5,1);   % allocate the right number of output arguments
...
for i = 1:nout
  ...
  [u,pi,b,phi0] = feval(it,u,pi,b,z,X,y,s2,B,pot,tau,G,outerExact,ninMVM,opts);
  ...
  if i<nout || nargout>3
    if outerExact                                            % exact computation
      if woodbury
        if output, fprintf('OUTER: Compute exact z using Woodbury\n'), end
        [z,ldA,varargout{:}] = diaginv_woodbury(X,1/s2,pi);   % use Woodbury
      else
        if output, fprintf('OUTER: Compute exact z using dense matrices\n'), end
        [z,ldA,varargout{:}] = diaginv_full(X,1/s2,B,pi);  % use full matrix
      end
    else                                    % approximate computation by Lanczos
      if output
        fprintf('OUTER: Approximate z using Lanczos (%d/%d)\n',noutMVM,n)
      end
      [z,ldA,varargout{:}] = diaginv_lanczos(X,1/s2,B,pi,noutMVM,output);
    end  
  end
  ...
  if cvg || i==nout % converged or last loop
    ...
    m = postmean(X,s2,B,pi,y,b,G,outerExact,ninMVM,u,output); ga = 1./pi;
    break
  end
end
...
function [u,pi,b,phi0] = innerVB(u,pi,b,z,X,y,s2,B,pot,tau,G,exact,ninMVM,opts)
  ...
  if G==1                    % PLS, no ga dependence since implicitely minimised
    ...
    [u,phi_z] = feval(pls,u,X,y,B,pls_opts,s2,'penVBnormsplit',pot,tau,z,G);
  end
  phi0 = phi_z - sum(z.*pi);            % phi0 = phi_[z=0] at the optimum (pi,u)
  if G==1
    [junk,junk,junk,b,pi] = penVB(B*u,pot,tau,z);
  else
    [junk,junk,junk,b,pi] = penVBnormsplit(B*u,pot,tau,z,G);
  end
...
function m = postmean(X,s2,B,pi,y,b,G,exact,nMVM,u,output)        % posterior mean
  ...
  if exact
    if woodbury
      ...
      if ~isscalar(G)
        m = linsolve_woodbury(X,1/s2,pi,X'*y/s2+B'*G'*b);          % Woodbury formula
      else
      ...
      if ~isscalar(G)
        m = linsolve_full(X,1/s2,B,pi,X'*y/s2+B'*G'*b);         % use the full matrix
      else
    ...
    if ~isscalar(G)
      m = linsolve_lcg(X,1/s2,B,pi,[X']*y/s2 + [B']*[G']*b,nMVM,u);
    else
